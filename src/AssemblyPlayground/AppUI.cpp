    ...
    if (instructions && instruction_count > 0) {
        ...
        while (clipper.Step()) {
            for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++) {
                ...
                if (ImGui::Selectable("##line", is_selected, ImGuiSelectableFlags_AllowDoubleClick | ImGuiSelectableFlags_SpanAllColumns, ImVec2(0, ImGui::GetTextLineHeight() * 1.5f)))
                {
                    //                    selected_address = is_selected ? -1 : insn.address;
                ...
                if (ImGui::BeginPopupContextItem("DisassemblyContextMenu")) {
                    ...
                    if (ImGui::MenuItem("Copy Address")) {
                        char buf[16];
                        snprintf(buf, sizeof(buf), "0x%llX", (long long unsigned)insn.address);
                ...
                // ## Memory Address
                ImGui::SameLine();
                ImGui::TextColored(ImVec4(0.2f, 0.8f, 1.0f, 1.0f), "0x%llX", (long long unsigned)insn.address);
                
                // ## PC ICON
                ImGui::SameLine();
                ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), is_current_pc ? "->" : "  ");
                
                // ## Instruction
                ImGui::SameLine();
                ImGui::TextColored(is_current_pc ? ImVec4(1.0f, 1.0f, 0.0f, 1.0f) : ImVec4(1.0f, 1.0f, 1.0f, 1.0f),
